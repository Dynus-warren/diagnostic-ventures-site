...
export default function SolutionPage() {
  const steps = [
    ...
    ["02", "Spin", "Spin the sample at up to 3,000 RPM for 20 seconds.", "/media/isolation-step-2.png"],
    ["03", "Separate", "Separate target pathogens from background interferents using sedimentation forces.", "/media/isolation-step-3.png"],
  ...
  return <main>
    ...
    <section className="workflow-section"><div className="shell"><SectionHeading eyebrow="Product workflow" title="Rapid centrifugal isolation." /><div className="workflow-grid">{steps.map(([n,title,text,image]) => <article key={n}><span>{n}</span><div className="workflow-image"><Image src={image} alt={`${title} step illustration`} width={219} height={158} sizes="(max-width: 700px) 50vw, 219px" /></div><h3>{title}</h3><p>{text}</p></article>)}</div></div></section>
